...
for i=1:size(x,2)  
    ft1(i)=fmTriangular(x(i),7,14,21); 
    ft2(i)=fmTriangular(x(i),14,14,22); 
    ft3(i)=fmTriangular(x(i),14,21,21); 
    
    fT1(i)=fmTrapezoidal(x(i),7,14,20,40); 
    fT2(i)=fmTrapezoidal(x(i),14,14,15,30); 
    fT3(i)=fmTrapezoidal(x(i),5,13,22,22); 

    fg1(i)=fmGaussiana(x(i),14,1); 
    fg2(i)=fmGaussiana(x(i),14,5); 
    fg3(i)=fmGaussiana(x(i),14,10); 
    
    fc1(i)=fmCampana(x(i),14,5,1); 
    fc2(i)=fmCampana(x(i),14,5,2); 
    fc3(i)=fmCampana(x(i),15,5,3); 
##    
